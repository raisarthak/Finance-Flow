...
module.exports = async function seedInMemory() {
  // Clear existing data
  await Promise.all([
    User.deleteMany({}),
    Account.deleteMany({}),
  ...
  // Calculate spent amounts from current month transactions
  ...
  await Budget.create({
    ...
    totalLimit: 5500,
    categories: [
      { name: 'Food & Dining', limit: 800, spent: spentByCategory['Food & Dining'] || 0, color: '#ef4444' },
      { name: 'Transport', limit: 300, spent: spentByCategory['Transport'] || 0, color: '#f97316' },
      { name: 'Shopping', limit: 500, spent: spentByCategory['Shopping'] || 0, color: '#eab308' },
      { name: 'Utilities', limit: 400, spent: spentByCategory['Utilities'] || 0, color: '#22c55e' },
      { name: 'Entertainment', limit: 200, spent: spentByCategory['Entertainment'] || 0, color: '#3b82f6' },
      { name: 'Healthcare', limit: 300, spent: spentByCategory['Healthcare'] || 0, color: '#8b5cf6' },
      { name: 'Rent', limit: 2200, spent: spentByCategory['Rent'] || 0, color: '#6366f1' },
      { name: 'Personal Care', limit: 150, spent: spentByCategory['Personal Care'] || 0, color: '#f43f5e' },
      { name: 'Education', limit: 200, spent: spentByCategory['Education'] || 0, color: '#ec4899' },
      { name: 'Insurance', limit: 450, spent: spentByCategory['Insurance'] || 0, color: '#14b8a6' }
    ]
  });
  console.log('   💰 Created budget with real spending data');

  // Create investments
  await Investment.create([
    {
      userId: user._id, name: 'Apple Inc.', symbol: 'AAPL', type: 'stocks',
      purchasePrice: 150.25, currentPrice: 198.50, quantity: 50,
      purchaseDate: new Date(2024, 2, 15), notes: 'Long-term hold'
    },
    {
      userId: user._id, name: 'Tesla Inc.', symbol: 'TSLA', type: 'stocks',
      purchasePrice: 245.00, currentPrice: 218.75, quantity: 20,
      purchaseDate: new Date(2024, 5, 10), notes: 'Growth stock'
    },
    {
      userId: user._id, name: 'Vanguard S&P 500 ETF', symbol: 'VOO', type: 'mutual_funds',
      purchasePrice: 420.00, currentPrice: 478.50, quantity: 30,
      purchaseDate: new Date(2023, 8, 1), notes: 'Index fund - core holding'
    },
    {
      userId: user._id, name: 'Bitcoin', symbol: 'BTC', type: 'crypto',
      purchasePrice: 42000.00, currentPrice: 67500.00, quantity: 0.5,
      purchaseDate: new Date(2024, 0, 15), notes: 'Crypto allocation'
    },
    {
      userId: user._id, name: 'Gold ETF', symbol: 'GLD', type: 'gold',
      purchasePrice: 185.00, currentPrice: 215.30, quantity: 40,
      purchaseDate: new Date(2023, 11, 1), notes: 'Hedge against inflation'
    },
    {
      userId: user._id, name: 'Fixed Deposit - HDFC', symbol: 'FD', type: 'fixed_deposit',
      purchasePrice: 10000.00, currentPrice: 10750.00, quantity: 1,
      purchaseDate: new Date(2024, 3, 1), notes: '7.5% APY - 1 year term'
    }
  ]);
  ...
  await Notification.create([
    {
      userId: user._id, type: 'salary', title: 'Salary Received',
      message: 'Your monthly salary of $8,500 has been credited to Primary Checking.',
      isRead: false, priority: 'medium', icon: 'dollar-sign'
    },
    {
      userId: user._id, type: 'budget_alert', title: 'Budget Alert: Shopping',
      message: "You've spent 85% of your shopping budget this month. $75 remaining.",
      isRead: false, priority: 'high', icon: 'alert-triangle'
    },
    {
      userId: user._id, type: 'investment', title: 'Investment Update',
      message: 'Your Apple (AAPL) holdings are up 32.1% since purchase. Consider rebalancing.',
      isRead: false, priority: 'low', icon: 'trending-up'
    },
    {
      userId: user._id, type: 'bill_due', title: 'Bill Reminder',
      message: 'Your credit card payment of $2,340.50 is due in 5 days.',
      isRead: false, priority: 'high', icon: 'credit-card'
    },
    {
      userId: user._id, type: 'unusual_spending', title: 'Unusual Spending Detected',
      message: 'Your entertainment spending is 40% higher than last month.',
      isRead: true, priority: 'medium', icon: 'alert-circle'
    },
    {
      userId: user._id, type: 'achievement', title: 'Savings Milestone! 🎉',
      message: 'Congratulations! Your savings have crossed $45,000. Keep it up!',
      isRead: true, priority: 'low', icon: 'award'
    }
  ]);
